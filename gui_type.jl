function gui_type(v8::Matrix{Float64},v9::Matrix{Any})
  ...
  bind(bhelp[1], "command", path -> Messagebox(title="Help", message="The ratio of the total number of cells given over to that type upon the start of the simulation. Ratios are normalised and as such do not have to sum to one."))
  bind(bhelp[2], "command", path -> Messagebox(title="Help", message="The rate at which a cells area will increase per timestep."))
  bind(bhelp[3], "command", path -> Messagebox(title="Help", message="The ratio of current area to initial area at which a cell division event will occur."))
  bind(bhelp[4], "command", path -> Messagebox(title="Help", message="The average distance a cell can travel in one step. Uses the same arbitrary units as other distance variables."))
  bind(bhelp[5], "command", path -> Messagebox(title="Help", message="The average radius of cells at the start of a simulation. This is also used in calculating cell division."))
  bind(bhelp[6], "command", path -> Messagebox(title="Help", message="This specifies a value which multiplies a cells proposed move direction in response to its detected ligand concentration. A value of 1 indicates that a cell will travel towards a source, whilst -1 will repel the cell. Note that specified randomness will dilute this effect."))
  ...
  bind(bhelp[8], "command", path -> Messagebox(title="Help", message="The chance that at any time step, the cell in question will die."))
  ...
  bind(bhelp[14], "command", path -> Messagebox(title="Help", message="Selecting this option will initialise all cells of the particular time close to the left wall of the environment."))
  bind(bhelp[15], "command", path -> Messagebox(title="Help", message="Selecting this option will allow this type of cell to have stem cell behaviour. A stem cell will change the type of its progeny depending upon its local ligand concentration and the stem threshold specified in the main menu. Progeny will be either of the same type (stem cell replication), or the type below (1 -> 2 -> 3 -> 4). Note that type 4 cells cannot be stem cells. Deselecting this option means that all types will only produce the same type."))
  bind(bhelp[16], "command", path -> Messagebox(title="Help", message="If the detected mean ligand concentration is above the stem threshold, the cells speed will be divided by 10, effectively sticking it in place."))
