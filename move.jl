...
function solve_overlap(m::Int, startloc::Point,walls,fc)
	...
	k = is_overlap(m::Int, startloc::Point,walls,fc)	
	if(k!=m)
...
##########################################################################################################
function is_overlap( m::Int, startloc::Point,walls,fc)
#k is used if we want to exclude also another cell than the studied cell (m)  
  ...
  for i in 1:length(alive_cells)
    ...
    if (alive_cells[i].x - alive_cells[m].x)^2 + (alive_cells[i].y - alive_cells[m].y)^2< (0.99*(alive_cells[i].r + alive_cells[m].r)) ^ 2
	  if  (i != m)
  ...
  if(length(index)>0)
	...
	if(counter_overlap<10)		  
	  is_overlap(m, startloc,walls,fc)
	  check_borders!(alive_cells[m],startloc,wall_behaviour,fc_behaviour,walls,fc)
	end
