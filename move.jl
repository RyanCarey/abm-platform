...
function move!(alive_cells::Vector{Cell},categories::Vector{Cell_type},dying_indices::Vector{Int},index::Int,
               x_size::Real, y_size::Real, border_settings::Vector{ASCIIString}, time::Real)
  ...
  if type_source=="Point"
	...
	dying_indices = solve_overlap(categories,m,startloc, dying_indices,x_size, y_size, border_settings,alive_cells)
...
function solve_overlap(categories::Vector{Cell_type},m::Int, startloc::Point, 
	...
	if k!=m
	  ...
	  if remaining_distance / categories[alive_cells[m].cell_type].avg_speed > minimum_ratio   #We can now make the cells to move
		...
		dying_indices=solve_overlap(categories, k,startlock,dying_indices,x_size, y_size, border_settings,alive_cells)
		...
		dying_indices=solve_overlap(categories, m,startlocm,dying_indices,x_size, y_size, border_settings,alive_cells)
		#if d_move <0
		else
...
function find_center_where_they_touch(cellm::Cell,cellk::Cell,startloc::Point)
	...
	r1 = cellm.r
	r2 = cellk.r
	theta1=mod2pi(cellm.angle)
	theta = mod2pi(acos((cellm.x - x1)/sqrt((cellm.x - x1)^2+(cellm.y - y1)^2))*sign(cellm.y - y1))
