...
function move_any!()
	...
	if(overlap)
	for i in 1:length(alive_cells)
	  #=if(is_overlap3(j,Point(alive_cells[j].x,alive_cells[j].y))!=j)
	  i=is_overlap3(j,Point(alive_cells[j].x,alive_cells[j].y))
	  println("alive_cells[$i].angle: ",alive_cells[i].angle)
	  println("alive_cells[$i].x: ",alive_cells[i].x)
	  println("alive_cells[$i].y: ",alive_cells[i].y)
	  println("alive_cells[$i].r: ",alive_cells[i].r)
	  println("alive_cells[$i].speed: ",alive_cells[i].speed)
	  println("")
	  println("alive_cells[$j].angle: ",alive_cells[j].angle)
	  println("alive_cells[$j].x: ",alive_cells[j].x)
	  println("alive_cells[$j].y: ",alive_cells[j].y)
	  println("alive_cells[$j].r: ",alive_cells[j].r)
	  println("alive_cells[$j].speed: ",alive_cells[j].speed)
	  println("distance: ", sqrt((alive_cells[i].x-alive_cells[j].x)^2+(alive_cells[i].y-alive_cells[j].y)^2)-alive_cells[j].r-alive_cells[i].r)
	  #pause(0)
	  end=#
	  alive_cells[i].x=x[i]
	  alive_cells[i].y=y[i]
...
function solve_overlap(m::Int, startloc::Point)
	...
	if(is_overlap3(m,startloc)!=m)
	  i=is_overlap3(m,startloc)
	  println("alive_cells[$i].angle: ",alive_cells[i].angle)
	  println("alive_cells[$i].x: ",alive_cells[i].x)
	  println("alive_cells[$i].y: ",alive_cells[i].y)
	  println("alive_cells[$i].r: ",alive_cells[i].r)
...
function is_overlap2(k)
	 for i in 1:length(alive_cells)
		if(i!=k)
			if (alive_cells[i].x - alive_cells[k].x) ^ 2 + (alive_cells[i].y - alive_cells[k].y) ^ 2 < 0.999*(alive_cells[i].r + alive_cells[k].r) ^ 2
			    return i
			end
		end
	end
...
##########################################################################################################
function is_overlap3(k,startloc)
	 for i in 1:length(alive_cells)
		if(i!=k)
