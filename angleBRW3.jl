...
function angleBRW2(ML=ones(50,50),R=[10,10,0.5],degree_precision=360)
  r = R[3]
	sum_x=0
	sum_y=0
	sum_norm_vect=0
    ...
    print("ligands",x_ligand_i,y_ligand_i)
	...
	if(sum_x!=0)
		...
		println(var)
		chosen_angle=beta+(rand()-1/2)*var*var
	elseif(sum_y!=0)
...
function anglePRW(prev_angle=-pi,variance=0.1)
	angle = prev_angle+randn()*variance
	return angle
end

#Persistent Biased Random walk
function anglePBRW(prev_angle,L,r,variance=0.25,omega=0.5,R=[0.5,0.5,0.5])
